...
fn setup_ring(num_decoys: usize) -> (Scalar, Vec<RistrettoPoint>) {
    ...
    let mut ring: Vec<RistrettoPoint> = (0..num_decoys)
        .map(|_| RistrettoPoint::random(&mut csprng))
        .collect();
    
    // Insert the signer's key at a random position
    let secret_index = csprng.next_u32() as usize % (num_decoys + 1);
